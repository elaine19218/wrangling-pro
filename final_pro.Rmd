...
# Transportation
  In New York City, because of issues like congestion, parking, and crowded city, most of people do not drive. Citi Bike is New York City’s bike share system, and the largest in the nation. So I choose to use the history data they provided to estimate the traveling situation. As the datasets are too large and even cannot be uploaded to cloud, I just chose to compare the same period time in two years. They are trip data in March 2019 and in March 2020.
```{r echo=FALSE}
...
Further more, after spliting the related streets name in original one column, I extracted all the sreets name from data. By combining frequency for every street in both start and stop column, I got frequency for all the streets in 2019 and 2020. To get a better illustration, top 10 streets are reserved. Then, under the help of function gather(), I got lines plot like this.  
From the plot we can easily see that, stations on the same hot streets in 2020 are used less frequent than 2019.
```{r}
##19
...
# Weather
Since the webpage where I intended to scrape the historical monthly weather information changed it's format now, I 
```{r echo=FALSE}
...
Here are some headlines I scraped from Google headlines. Although the node is quite easy to find, the cleaning process is really hard. Since there's many buttons on the page and different buttons have different name in the raw webpage I scraped, it's really time consuming to figure out every type of buttons representitives and make their regular expressions in codes.  
The output below shows us at this time how many of headlines mentioned covid. We can see that even the coronavirus outbreak has been happening for almost a half year, it is still a hot topic, which is consisitent with the ever-growing number of confired cases.
```{r}
